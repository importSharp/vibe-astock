...
<p align="center">
  <b>⚠️ 默认复盘只做公开盘面数据的整理、统计与市场层面研判，不推荐个股、不预测涨跌、不给买卖时机。<br>
  可选的个人决策辅助只提供可解释规则排序与群体历史统计，不提供个体涨跌预测或交易指令。<br>
...
**默认模式不做什么**：不推荐个股、不给参与倾向、不给买卖点位。个股一律只作客观陈述
（属于哪个题材、几板、龙虎榜谁在买、技术位置如何）。方向与情绪判断做到板块层面为止。
个人决策辅助是默认关闭的独立实验功能，不改变原始复盘产物。
...
复盘页提供一个**默认折叠、按需启用**的个人决策辅助面板。它不会改写原始复盘，
也不让 LLM 直接猜涨跌；只把目标日收盘后已经确定的事实按固定规则整理成：

- 市场环境分与进攻 / 谨慎 / 防守 / 回避窗口；
- 涨停样本的市场、题材协同、封板质量、流动性、历史统计五项规则分；
- ST、尾盘才封、反复开板、板块孤立、疑似一字板等硬性风险否决；
- 同类规则的历史胜率、收益均值和样本数（不足 30 个样本不显示胜率）。

⚠️ 排名是**次一交易日的观察优先级**，不是买卖指令；同类历史胜率是群体统计，
不是某只股票的个体上涨概率。系统允许输出“没有合格候选”，不要为了排名而强行选择。
接口为 `GET /api/review/decision-assist?date=YYYY-MM-DD`，只有展开面板时才会计算。
